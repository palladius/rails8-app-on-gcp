...
- **`pseudocode`**:
  ```bash
  gcloud auth login $GCP_EMAIL && gcloud auth application-default login
...
- **`pseudocode`**:
  ```bash
  cp .env.dist .env && vim .env # set GCP_EMAIL
  docker compose up -d
...
- **`postrequisites`**:
  ...
  - Admin account bootstrapped from GCP_EMAIL
...
- **`evals`**:
  ...
  - `[RUBY]` Verify db/seeds.rb enforces GCP_EMAIL presence
...
- **`pseudocode`**:
  ...
  gcloud run deploy blog --source . --region us-central1 --allow-unauthenticated --set-env-vars GCP_EMAIL=$GCP_EMAIL
