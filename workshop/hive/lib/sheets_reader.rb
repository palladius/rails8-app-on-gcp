...
module WorkshopHive
  class SheetsReader
    ...
    def self.fetch_entries(sheet_id: ENV["HIVE_SPREADSHEET_ID"], credentials: nil)
      ...
      warn "[SheetsReader] Error fetching from Google Sheets: #{e.message}. Falling back to cached/mock data."
      @cache || MOCK_ENTRIES
    end
    ...
    def self.parse_rows(rows)
      ...
      url_idx = header.index { |h| h.include?("url") || h.include?("run") } || 2
      step_idx = header.index { |h| h.include?("step") || h.include?("level") } || 3

      rows[1..].map do |row|
        step_str = row[step_idx].to_s.strip
        {
          nickname: row[nick_idx].to_s.strip,
          url: row[url_idx].to_s.strip,
          step: step_str,
          step_number: extract_step_number(step_str),
          timestamp: row[ts_idx].to_s.strip
        }
      end.reject { |e| e[:url].empty? }
    end

    def self.extract_step_number(step_str)
