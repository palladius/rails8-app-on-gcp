...
- 🛡️ **Defensive Frontend String Coercion**:
  - Fixed `escapeHtml` to coerce numbers (such as GHI issue IDs) to String, preventing a `TypeError: str.replace is not a function`.
  - Fixed `formatHHMM` in `public/js/hive.js` to coerce timestamp inputs to String before matching regex.
