# 📚 Project Sources & Documentation Index

A curated index of authoritative upstream documentation, architectural references, and specifications used in the **Rails 8 on Google Cloud** project and workshop.

---

## ☁️ Google Cloud Platform (GCP)

- ☁️ **Google Cloud Official Platform**:  
  https://cloud.google.com/

### Cloud Run & Containers
- 🐳 **Deploying Multi-Container Services / Docker Compose to Cloud Run**:  
  https://docs.cloud.google.com/run/docs/deploy-run-compose
- 🚢 **Cloud Run Multi-Container Pods (Sidecars Specification)**:  
  https://cloud.google.com/run/docs/deploying-multi-container
- ⚙️ **Cloud Run Jobs Overview**:  
  https://cloud.google.com/run/docs/create-jobs

### Infrastructure as Code (IaC)
- 🏗️ **Terraform on Google Cloud**:  
  https://docs.cloud.google.com/docs/terraform

### Storage & Security
- 🪣 **Google Cloud Storage (GCS) Overview**:  
  https://cloud.google.com/storage
- 🪣 **Google Cloud Storage (GCS) IAM-signed URLs without Service Account Keys (`signBlob`)**:  
  https://cloud.google.com/storage/docs/access-control/signing-urls-with-helpers
- 🔒 **Cloud SQL Auth Proxy Overview & Mutual TLS (mTLS)**:  
  https://docs.cloud.google.com/sql/docs/mysql/sql-proxy
- 🔑 **Google Cloud Secret Manager Overview**:  
  https://docs.cloud.google.com/secret-manager/docs/overview

### AI & Observability
- 🤖 **Google Antigravity Download & IDE**:  
  https://antigravity.google/download
- 🍌 **Google GenAI / Vertex AI Gemini API**:  
  https://cloud.google.com/vertex-ai/generative-ai/docs/learn/overview
- 🐝 **Google Cloud Logging & Error Reporting**:  
  https://cloud.google.com/logging/docs

---

## 💎 Ruby on Rails 8

- 🚀 **Rails 8: No PaaS Required (Official Announcement)**:  
  https://rubyonrails.org/2024/11/7/rails-8-no-paas-required
- 📦 **Rails 8 Release Notes**:  
  https://guides.rubyonrails.org/8_0_release_notes.html
- 💼 **Active Job Basics Guide**:  
  https://guides.rubyonrails.org/active_job_basics.html
- ⚡ **Solid Queue (Database-backed ActiveJob runner)**:  
  https://github.com/rails/solid_queue
- 🗄️ **Solid Cache (Database-backed Cache store)**:  
  https://github.com/rails/solid_cache
- 🔌 **Solid Cable (Database-backed Action Cable pubsub)**:  
  https://github.com/rails/solid_cable
- 🖼️ **ActiveStorage Overview & GCS Service**:  
  https://guides.rubyonrails.org/active_storage_overview.html

---

## 👥 Authors & Maintainers

- **Riccardo Carlesso** 🦖
  - GitHub: https://github.com/palladius
  - LinkedIn: https://linkedin.com/in/riccardocarlesso
- **Emiliano Della Casa** 🍝🏎️
  - GitHub: https://github.com/emilianodellacasa
  - LinkedIn: https://www.linkedin.com/in/emilianodellacasa
